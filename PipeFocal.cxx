...
//
  ...
  // voRB24->AddNode(voRB24CuTubeM, 1, gGeoIdentity); // MvL: remove A-side Cu tube at ~5m; interferes with 9m position of FOCAL
  // voRB24->AddNode(voRB24CuTubeA, 1, gGeoIdentity); // MvL: remove air volume with high thresholds for FOCAL
  z = - kRB24CuTubeL/2 + kRB24CuTubeFL/2.;
  // Remove Flange in RB24 for FOCAL 7m
  ...
  // MvL: remove for FOCAL 8m tests: Bellow, ion pump, Valve and pump port ('warm unit')
  ...
  //voRB24->AddNode(voRB242, 1, new TGeoTranslation(0., 0., z));  // MvL: remove move beampipe and flanges; interferes with FOCAL @ 9m
  //
  //   RB24/3
  //
  // Copper Tube RB24/3
...
//___________________________________________
void PipeFocal::createMaterials()
{
  //
  ...
  matmgr.Material("PIPEFOCAL", 2,  "SILICON$",  28.09 , 14.00 , 2.33 , 9.36 , 45.); //?
  //
  //     Berillium
  matmgr.Material("PIPEFOCAL", 5, "BERILLIUM$", 9.01, 4., 1.848, 35.3, 36.7);
  //
  //     Carbon
  matmgr.Material("PIPEFOCAL", 6,  "CARBON$   ", 12.01, 6., 2.265, 18.8, 49.9);
  //
  //     Aluminum
  matmgr.Material("PIPEFOCAL", 9,  "ALUMINIUM$", 26.98, 13., 2.7, 8.9, 37.2);
  //
  //     Copper
  matmgr.Material("PIPEFOCAL", 10, "COPPER1", 63.55, 29, 8.96, 1.43, 85.6/8.96);
  //
  //     Air
  matmgr.Mixture("PIPEFOCAL", 15, "AIR$      ", aAir, zAir, dAir, 4, wAir);
  matmgr.Mixture("PIPEFOCAL", 35, "AIR_HIGH$ ", aAir, zAir, dAir, 4, wAir);
  //
  //     Vacuum
  matmgr.Mixture("PIPEFOCAL", 16, "VACUUM$ ", aAir, zAir, dAir1, 4, wAir);
  //
  //     stainless Steel
  matmgr.Mixture("PIPEFOCAL", 19, "STAINLESS STEEL$", asteel, zsteel, 7.88, 4, wsteel);
  //
  //     reduced density steel to approximate pump getter material
  matmgr.Mixture("PIPEFOCAL", 20, "GETTER$", asteel, zsteel, 1.00, 4, wsteel);
  //     Al-Be alloy
  //
  matmgr.Mixture("PIPEFOCAL", 21, "AlBe$", aAlBe, zAlBe, 2.07, 2, wAlBe);
  //     Polyamid
  //
  matmgr.Mixture("PIPEFOCAL", 22, "PA$", aPA, zPA, 1.14, -4, wPA);
  //
  //     Kapton
  matmgr.Mixture("PIPEFOCAL", 23, "KAPTON", aKapton, zKapton, dKapton, 4, wKapton);
  // Anticorodal
  matmgr.Mixture("PIPEFOCAL", 24, "ANTICORODAL", aaco, zaco, 2.66, 3, waco);

  //
  //     Insulation powder
  matmgr.Mixture("PIPEFOCAL", 14, "INSULATION0$", ains, zins, 0.41, 4, wins);
  matmgr.Mixture("PIPEFOCAL", 34, "INSULATION1$", ains, zins, 0.41, 4, wins);
  matmgr.Mixture("PIPEFOCAL", 54, "INSULATION2$", ains, zins, 0.41, 4, wins);

  //    NEG
  matmgr.Mixture("PIPEFOCAL", 25, "NEG COATING", aNEG, zNEG, dNEG, -3, wNEG);

  //---------------------------------
  //  Aluminium AA5083 for MFT
  matmgr.Mixture("PIPEFOCAL", 63, "ALUMINIUM5083$",aALU5083,zALU5083, 2.66 ,4,wALU5083); // from aubertduval.fr
  // Aluminium AA2219 for MFT
  matmgr.Mixture("PIPEFOCAL", 64, "ALUMINIUM2219$",aALU2219,zALU2219, 2.84 ,6,wALU2219); // from aubertduval.fr
  //---------------------------------
  ...
  // Silicon for ITS UPGRADE
  matmgr.Medium("PIPEFOCAL", 2,  "SILICON",   2, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin); //?

  //    Beryllium

  matmgr.Medium("PIPEFOCAL", 5, "BE",       5, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

  //    Carbon
  matmgr.Medium("PIPEFOCAL", 6, "C",        6, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Aluminum
  matmgr.Medium("PIPEFOCAL", 9, "ALU",      9, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //    Copper
  matmgr.Medium("PIPEFOCAL", 10, "CU",      10, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Air
  matmgr.Medium("PIPEFOCAL", 15, "AIR",     15, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  matmgr.Medium("PIPEFOCAL", 35, "AIR_HIGH",35, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Vacuum
  matmgr.Medium("PIPEFOCAL", 16, "VACUUM", 16, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //matmgr.Medium(16, "VACUUM", 16, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Steel
  matmgr.Medium("PIPEFOCAL", 19, "INOX",   19, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //    Getter
  matmgr.Medium("PIPEFOCAL", 20, "GETTER", 20, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //   AlBe - Aloy
  matmgr.Medium("PIPEFOCAL", 21, "AlBe"  , 21, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //   Polyamid
  matmgr.Medium("PIPEFOCAL", 22, "PA"  ,   22, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //   Antocorodal
  matmgr.Medium("PIPEFOCAL", 24, "ANTICORODAL",   24, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //    Insulation Powder
  matmgr.Medium("PIPEFOCAL", 14, "INS_C0", 14, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  matmgr.Medium("PIPEFOCAL", 34, "INS_C1", 34, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  matmgr.Medium("PIPEFOCAL", 54, "INS_C2", 54, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //
  //   KAPTON
  matmgr.Medium("PIPEFOCAL", 23, "KAPTON", 23, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

  //
  //   NEG
  matmgr.Medium("PIPEFOCAL", 25, "NEG COATING", 25, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);

  //----------------- for the MFT ----------------------
  matmgr.Medium("PIPEFOCAL", 63,"AA5083", 63, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  matmgr.Medium("PIPEFOCAL", 64,"AA2219", 64, 0, isxfld, sxmgmx, tmaxfd, stemax, deemax, epsil, stmin);
  //----------------------------------------------------

}

TGeoPcon* PipeFocal::MakeMotherFromTemplate(const TGeoPcon* shape, Int_t imin, Int_t imax, Float_t r0, Int_t nz)
{
  //
...
TGeoPcon* PipeFocal::MakeInsulationFromTemplate(TGeoPcon* shape)
{
  //
...
TGeoVolume* PipeFocal::MakeBellow(const char* ext, Int_t nc, Float_t rMin, Float_t rMax, Float_t dU, Float_t rPlie, Float_t dPlie)
{
  ...
  const TGeoMedium* kMedVac    =  gGeoManager->GetMedium("PIPEFOCAL_VACUUM");
  //const TGeoMedium* kMedSteel  =  gGeoManager->GetMedium("PIPEFOCAL_INOX");
  const TGeoMedium* kMedAlu5083 =  gGeoManager->GetMedium("PIPEFOCAL_AA5083"); //fm
...
// ----------------------------------------------------------------------------
FairModule* PipeFocal::CloneModule() const { return new PipeFocal(*this); }
ClassImp(o2::passive::PipeFocal);
