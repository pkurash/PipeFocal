...
//-------------------------------------------------------------------------
//  Beam pipe geometry class for FOCAL upgrade simulations
//  Based on class for ALICE MFT upgrade  (AliPIPEv4 in standard AliRoot)
...
#include "AliConst.h"
#include "AliMagF.h"
#include "AliPIPEFOCAL.h"
#include "AliRun.h"
#include "AliLog.h"

ClassImp(AliPIPEFOCAL)

//_____________________________________________________________________________
AliPIPEFOCAL::AliPIPEFOCAL():
...
//_____________________________________________________________________________
AliPIPEFOCAL::AliPIPEFOCAL(const char *name, const char *title)
  : AliPIPE(name,title),
    fRmax(1.8),              // outer radius of Be beam pipe (central part)
    fR2(2.4),                // outer radius of Be beam pipe (large R part)
    fBe(0.08),               // width of Be beam pipe
    fZc(330.0),              // end of conical section (A side)
    fZ1(240.0),              // beginning of beam pipe z location (A side)
    fZ2(-44.4),              // end of Be beam pipe z location (C side)
    fZflange(480)            // location of flange (change to R = 4cm or so)
{
  // Constructor
  TString titlestr(title);
  if (titlestr.Contains("rap6")) {
     fZ1 = 100;
     fZc = 102;
     fZflange = 820;
  }
...
void AliPIPEFOCAL::CreateGeometry()
{
  AliDebug(1,"Create PIPEv4 geometry");


  AliInfo(Form("Setting up FOCAL beampipe conical section starts at %.1f and ends at %.1f ; flange at %.1f cm",fZ1,fZc,fZflange));
  //
...
//___________________________________________
void AliPIPEFOCAL::CreateMaterials()
{
  //
...
TGeoPcon* AliPIPEFOCAL::MakeMotherFromTemplate(TGeoPcon* shape, Int_t imin, Int_t imax, Float_t r0, Int_t nz)
{
  //
...
TGeoPcon* AliPIPEFOCAL::MakeInsulationFromTemplate(TGeoPcon* shape)
{
  //
...
TGeoVolume* AliPIPEFOCAL::MakeBellow(const char* ext, Int_t nc, Float_t rMin, Float_t rMax, Float_t dU, Float_t rPlie, Float_t dPlie)
{
